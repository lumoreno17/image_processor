...
void ImageProcessor::getMask(cv::Mat input_img)
{
  ...
  inRange(blur_img, Scalar(LOWER_YELLOW_1, LOWER_YELLOW_2, LOWER_YELLOW_3),
...
void ImageProcessor::convertRGBToHSV(cv::Mat input_img)
{
  cvtColor(input_img, hsv_img, COLOR_BGR2HSV);
}

void ImageProcessor::convertRGBToHLS(cv::Mat input_img)
{
  cvtColor(input_img, hls_img, COLOR_RGB2HLS);
}

void ImageProcessor::convertRGBToLAB(cv::Mat input_img)
{
  cvtColor(input_img, lab_image, COLOR_RGB2Lab);
}
...
void ImageProcessor::printImages() 
{
  imshow("Original", original_img);
  imshow("HLS", hls_img);
  imshow("HSV", hsv_img);
...
bool ImageProcessor::processImage(cv::Mat image)
{
  original_img = image;
  convertRGBToHSV(original_img);
  convertRGBToHLS(original_img);
  convertRGBToLAB(original_img);
  getWarpedImage(original_img,20,20,60);
  getMask(warped_img);
  bool ret = getResultImage(warped_img);
  if(ret)
